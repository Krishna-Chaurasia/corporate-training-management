        ...
        <div id="courseFilter" style="display: none;">
            ...
            <select id="course" name="course">
				...
				try{
					...
					String query = "SELECT  ctp_proposals.*, ctp_courses.course_name FROM ctp_proposals INNER JOIN ctp_courses ON ctp_proposals.course_id = ctp_courses.course_id";
			...
			<tbody>
				...
				Class.forName("com.mysql.cj.jdbc.Driver");
				 Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/corporate_training_mgmt_db?useSSL=false",
						"root", "mysql");
				//PreparedStatement ps = conn.prepareStatement("select * from ctp_scheduling where faculty_id in (select registration_id from ctp_registration where email_id = ?)");
				
				
				
				
				PreparedStatement ps = conn.prepareStatement("SELECT ctp_scheduling.*, ctp_registration.registration_id, ctp_registration.f_name FROM ctp_scheduling INNER JOIN ctp_registration ON ctp_scheduling.faculty_id = ctp_registration.registration_id where faculty_id in (select registration_id from ctp_registration where email_id = ?)");

				ps.setString(1, uemail);
				System.out.println("Username from session: " + uemail); // Add this line for debugging
